...
def main():
    ...
    for line in open(treePath):
        line = line.rstrip()
        eid, indexP, indexC, Vec = line.split('\t')[0], line.split('\t')[1], int(line.split('\t')[2]), line.split('\t')[
            3]
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez(os.path.join(cwd,'data/Weibograph/'+id+'.npz'), x=x_x, x_pos=x_pos, root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
