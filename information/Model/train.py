...
def classify(treeDic, x_test  , x_train,TDdroprate,BUdroprate,lr, weight_decay,patience,n_epochs,batchsize,dataname,iter, fold_count):
    ...
    for unsup_epoch in range(10):
        ...
        train_loader = DataLoader(traindata_list, batch_size=batchsize, shuffle=False, num_workers=0)
    ...
    for epoch in range(n_epochs):
        ...
        early_stopping(np.mean(temp_val_losses), np.mean(temp_val_accs), np.mean(temp_val_F1), np.mean(temp_val_F2),
                       np.mean(temp_val_F3), np.mean(temp_val_F4),np.mean(temp_val_Prec1),np.mean(temp_val_Prec2),np.mean(temp_val_Prec3),np.mean(temp_val_Prec4),np.mean(temp_val_Recll1),np.mean(temp_val_Recll2),np.mean(temp_val_Recll3),np.mean(temp_val_Recll4),model, 'RDEA_'+str(fold_count)+'_', dataname)
        P1 = np.mean(temp_val_Prec1)
        P2 = np.mean(temp_val_Prec2)
        P3 = np.mean(temp_val_Prec3)
        P4 = np.mean(temp_val_Prec4)
        R1= np.mean(temp_val_Recll1)
        R2 = np.mean(temp_val_Recll2)
        R3 = np.mean(temp_val_Recll3)
        R4 = np.mean(temp_val_Recll4)
        ...
        if epoch>=199:
            ...
            P1 = early_stopping.P1
            P2 = early_stopping.P2
            P3 = early_stopping.P3
            P4 = early_stopping.P4

            R1 = early_stopping.R1
            R2 = early_stopping.R2
            R3 = early_stopping.R3
            R4 = early_stopping.R4
        if early_stopping.early_stop:
            ...
            R4 = early_stopping.R4
            break
    return train_losses , val_losses ,train_accs, val_accs,accs,F1,F2,F3,F4,P1,P2,P3,P4,R1,R2,R3,R4
...
for iter in range(iterations):
    ...
    t1 = time.time()
    train_losses, val_losses, train_accs, val_accs0, accs0, F1_0, F2_0, F3_0, F4_0,P1_0, P2_0, P3_0, P4_0,R1_0, R2_0, R3_0, R4_0 = classify(treeDic,
                                                                                               fold0_x_test,
                                                                                               fold0_x_train,
                                                                                               ...
                                                                                               datasetname,
                                                                                               iter, 0)
    train_losses, val_losses, train_accs, val_accs1, accs1, F1_1, F2_1, F3_1, F4_1,P1_1, P2_1, P3_1, P4_1, R1_1, R2_1, R3_1, R4_1 = classify(treeDic,
                                                                                               fold1_x_test,
                                                                                               fold1_x_train,
                                                                                               ...
                                                                                               datasetname,
                                                                                               iter, 1)
    train_losses, val_losses, train_accs, val_accs2, accs2, F1_2,F2_2, F3_2, F4_2,P1_2,P2_2, P3_2, P4_2,R1_2,R2_2, R3_2, R4_2 = classify(treeDic,
                                                                                               fold2_x_test,
                                                                                               fold2_x_train,
                                                                                               TDdroprate,BUdroprate, lr,
                                                                                               weight_decay,
                                                                                               ...
                                                                                               datasetname,
                                                                                               iter, 2)
    train_losses, val_losses, train_accs, val_accs3, accs3, F1_3, F2_3, F3_3, F4_3, P1_3, P2_3, P3_3, P4_3, R1_3, R2_3, R3_3, R4_3 = classify(treeDic,
                                                                                               fold3_x_test,
                                                                                               fold3_x_train,
                                                                                               TDdroprate,BUdroprate, lr,
                                                                                               weight_decay,
                                                                                               ...
                                                                                               datasetname,
                                                                                               iter, 3)
    train_losses, val_losses, train_accs, val_accs4, accs4, F1_4, F2_4, F3_4, F4_4,P1_4, P2_4, P3_4, P4_4,R1_4, R2_4, R3_4, R4_4 = classify(treeDic,
                                                                                               fold4_x_test,
                                                                                               fold4_x_train,
                                                                                               TDdroprate,BUdroprate, lr,
                                                                                               weight_decay,
    ...
    FR_P.append((P2_0 + P2_1 + P2_2 + P2_3 + P2_4) / 5)
    FR_R.append((R2_0 + R2_1 + R2_2 + R2_3 + R2_4) / 5)

    TR_F1.append((F3_0 + F3_1 + F3_2 + F3_3 + F3_4) / 5)
    TR_P.append((P2_0 + P2_1 + P2_2 + P2_3 + P2_4) / 5)
    TR_R.append((R2_0 + R2_1 + R2_2 + R2_3 + R2_4) / 5)
    UR_F1.append((F4_0 + F4_1 + F4_2 + F4_3 + F4_4) / 5)
    print("check  iter: {:04d} | aaaaaccs: {:.4f}".format(iter, test_accs[iter]))
    t2 = time.time()
    print("total time:")
    print(t2 - t1)
print("Total_Test_Accuracy: {:.4f}|NR F1: {:.4f}|FR F1: {:.4f} P: {:.4f} R: {:.4f}|TR F1: {:.4f} P: {:.4f} R: {:.4f}|UR F1: {:.4f}".format(
    sum(test_accs) / iterations, sum(NR_F1) /iterations, sum(FR_F1) /iterations,sum(FR_P) /iterations,sum(FR_R) /iterations, sum(TR_F1) / iterations, sum(TR_P) /iterations,sum(TR_R) /iterations,sum(UR_F1) / iterations))
